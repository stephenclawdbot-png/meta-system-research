...
def fetch_dexscreener_pairs():
    """Fetch trending/new pairs from DexScreener API"""
    try:
        # DexScreener trending/new pairs endpoint
...
def analyze_dexscreener_data(data):
    """Analyze DexScreener data for alpha gems in 30k-200k mcap range"""
...
"""
    ...
    if not gems:
        report += "🔍 No alpha gems detected in target range\nMarket may be quiet or scanning limitations encountered\n"
    else:
        report += f"🔥 TOP {min(len(gems), 8)} ALPHA GEMS ({len(gems)} total)\n" + "-" * 40 + "\n"
...
💡 Detecting alpha gems before mainstream attention
DYOR - High risk volatile assets"""
    
    return report

def main():
    print("🔍 Fetching live DexScreener data for alpha gems...")
    
    data = fetch_dexscreener_pairs()
    
    if data:
        # Save raw data for reference
        with open("dex_data_latest.json", "w") as f:
            json.dump(data, f, indent=2)
    
    gems = analyze_dexscreener_data(data)
    ...
    # Save report
    with open("cron_alpha_scanner_report_current.txt", "w") as f:
        f.write(report)
