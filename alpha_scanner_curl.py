#!/usr/bin/env python3
"""
Alpha Scanner - Query DexScreener for memecoins using curl
"""
...
def fetch_dexscreener_data():
    ...
    try:
        result = subprocess.run(
            ['curl', '-s', 'https://api.dexscreener.com/latest/dex/search/?q=solana'],
...
"""
        ...
        high_alpha_tokens = [coin for coin in memecoins if coin['alpha_score'] >= 80]
        if high_alpha_tokens:
            report += "💎 EXCEPTIONAL ALPHA GEMS 💎\n"
            for token in high_alpha_tokens:
                report += f"• {token['baseToken']['symbol']} - Score {token['alpha_score']}/100 (Vol/MCap: {token['vol_mcap_ratio']:.1f}%, Buy: {token['buy_ratio']:.1%})\n"
            report += "\n"
    
    report += """⚠️ DISCLAIMER: HIGH RISK / NOT FINANCIAL ADVICE
