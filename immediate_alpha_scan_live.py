...
def fetch_all_dexscreener_pairs():
    """Try to fetch recent/new pairs from DexScreener"""
    try:
        # Try the recent pairs endpoint
        url = "https://api.dexscreener.com/latest/dex/tokens/new?rankBy=volume"
...
def fetch_dexscreener_tokens_expanded():
    """Fetch tokens from DexScreener with expanded search"""
    all_tokens = []
    
    # More comprehensive list of search terms
    search_queries = [
        # Popular memecoins
        "PEPE", "DOGE", "SHIB", "BONK", "FLOKI", "WIF", "BOME", "MEME",
        "GUAC", "PUDGY", "POPCAT", "MOOK", "HARAMBE", "HODL", "HONEY",
        "SAFE", "RUG", "SHARK", "WHALE", "CAT", "FROG", "DOG", "MOON",
        "RICH", "SOL", "ETH", "BTC", "USDC", "DAI", "ASTR", "JOY",
        # Newer trends
        "BASED", "LOVE", "AI", "COIN", "TOKEN", "X", "420", "69", 
        "APE", "PUNK", "NINJA", "WIZARD", "WOLF", "BEAR", "BULL",
        # Simple terms that often appear in new tokens
        "NEW", "ONE", "ZERO", "FIRST", "LAST"
    ]
    
    print(f"🔍 Searching {len(search_queries)} terms on DexScreener...")
    ...
    for query in search_queries:
        try:
            ...
            if data and 'pairs' in data and data['pairs']:
                print(f"✓ Found {len(data['pairs'])} tokens for '{query}'")
                
                for token in data['pairs']:
                    mcap = token.get('fdv', 0)
                    
                    # Broaden filter to include more tokens (5k-500k)
                    if 5000 <= mcap <= 500000:
                        ...
                        # Lower threshold for transactions
    ...
    # Also try to get new pairs
    print("🔍 Fetching recent/new pairs...")
    recent_pairs = fetch_all_dexscreener_pairs()
    if recent_pairs:
        print(f"✓ Found {len(recent_pairs)} recent pairs")
        
        for token in recent_pairs:
            mcap = token.get('fdv', 0)
...
def main():
    print("🧠 MEMECOIN ALPHA SCANNER - EXPANDED SEARCH")
    print("=" * 60)
    print("Scan Time:", datetime.now().strftime("%A, %B %d, %Y — %I:%M %p (GMT+8)"))
    print("Target Range: $5k - $500k Market Cap")
    print("Metrics: Volume/MCap Ratio, Buy Pressure, Momentum, Age")
    print()
    
    # Fetch tokens
    tokens = fetch_dexscreener_tokens_expanded()
    ...
    # Filter for minimum quality (lower threshold for expansion)
    tokens = [t for t in tokens if t['alpha_score'] >= 10]
    
    # Sort by alpha score
    tokens.sort(key=lambda x: x['alpha_score'], reverse=True)
    
    print(f"🔥 Found {len(tokens)} potential alpha tokens")
    ...
    if tokens:
        ...
        # Filter for original 30k-200k target
        target_tokens = [t for t in tokens if 30000 <= t['mcap'] <= 200000]
        
        if target_tokens:
            print("🎯 TOKENS IN ORIGINAL 30K-200K RANGE:")
            print("-" * 40)
            for token in target_tokens:
                print(f"{token['symbol']} - MCap: ${token['mcap']:,} - Alpha: {token['alpha_score']}/100")
            print()
        ...
        if tokens:
            ...
            print(f"🎯 Target Range (30k-200k): {len(target_tokens)} tokens")
